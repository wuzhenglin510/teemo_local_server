...
function prepareTestEnv() {
    shell.mkdir('-p', path.join(ProjectRoot, 'raw/group'), path.join(ProjectRoot, 'raw/scenerio'));
        shell.mkdir('-p', path.join(ProjectRoot, 'build/test/group'), path.join(ProjectRoot, 'build/test/scenerio'));
        ...
        if (os.platform().indexOf("darwin") != -1) {
            shell.cp('-n', path.join(__dirname, '../../server/core/test_env/package.json'), path.join(ProjectRoot, 'build'))
            child_process.exec(`osascript -e 'tell application "Terminal" to do script "cd ${path.join(ProjectRoot, 'build')} && npm install"'`)
            ...
            shell.cp('-n', path.join(__dirname, '../../server/core/test_env/package.json'), path.join(ProjectRoot, 'build'))
            child_process.exec(`start cmd.exe @cmd /k "cd ${path.join(ProjectRoot, 'build')} & npm install"`)
